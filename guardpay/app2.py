...
from db import carregar_transacoes, conectar_db, inserir_transacoes_aleatorias, historico_por_cliente, inserir_transacoes_analise
from utils import gerar_transacoes_aleatorias, gerar_transacoes_analise
...
# Botão para gerar transações aleatorias
if st.button("Gerar Transações aleatorias"):
    transacoes = gerar_transacoes_aleatorias()
    conexao = conectar_db()
    inserir_transacoes_aleatorias(conexao,transacoes)
    st.success("Transações geradas com sucesso!")
    df = carregar_transacoes()

# Botão para gerar transações para analise
if st.button("Gerar Transações para analise"):
    transacoes = gerar_transacoes_analise()
    conexao = conectar_db()
